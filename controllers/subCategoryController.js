  ...
  async create(req, res, next) {
    try {
      ...
      if (!nameTm || !nameRu || !nameEn || !categoryId) {
        return next(
          ApiError.badRequest("Subkategoriýa ady giriz ýada kategoriýa ýalňyş!")
        );
      }
      const existCategory = await Category.findByPk(categoryId);
      if (!existCategory) {
        return next(ApiError.badRequest("Şeýle kategoriýa ýok!"));
      }
      // Сохраняем файл
      let fileName = null;
      if (file) {
      ...
      const hasSubCategory = await SubCategory.findOne({ where: { nameTm } });
      if (hasSubCategory) {
        return next(
          ApiError.badRequest(`${nameTm} subkategoriýa ulgamda hasaba alnan!`)
        );
      }
      // Создаем категорию
