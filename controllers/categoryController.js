...
class CategoryController {
  async create(req, res, next) {
    try {
      ...
      const hasCategory = await Category.findOne({ where: { nameTm } });
      if (hasCategory) {
        return next(
          ApiError.badRequest(`${nameTm} kategoriýa ulgamda hasaba alnan!`)
        );
      }
      // Создаем категорию
      ...
      return next(
        ApiError.internal(
          "Kategoriýa döretmekde ýalňyşlyk ýüze çykdy, duyduranyn ucin sag bol bratok" +
            e
        )
      );
    }
