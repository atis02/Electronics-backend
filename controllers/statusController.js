  ...
  async create(req, res, next) {
    try {
      ...
      if (!nameTm || !nameRu || !nameEn) {
        return next(ApiError.badRequest("Status ady giriz !"));
      }

      const hasStatus = await Status.findOne({ where: { nameTm } });
      if (hasStatus) {
        return next(
          ApiError.badRequest(`${nameTm} Status ulgamda hasaba alnan!`)
        );
      }
      // Создаем категорию
